...
$row = entries("SELECT Name, Surname, PhoneNumber, Company, Address, Web, Birthdate, Label, Nick FROM userdetail WHERE id = {$id} AND userID = {$_SESSION['userID']}")[0];
?>
...
<?php foreach ($colTypes as $colName=>$colType){  ?>
    ...
    <button hx-post='/changerow.php?id=<?=$id?>' hx-target='#e<?=$id?>' hx-swap='outerHTML' hx-include='.includefor<?=$id?>' class="rounded-md bg-blue-400 p-2">✔</button>
    </td>
    <td class="font-medium py-3.5 px-5 text-gray-700  text-left ">
